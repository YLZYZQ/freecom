...
/// <summary>协议帮助：格式定义 + 示例 + 下位机 C 例程（代码块可选中复制）。</summary>
public partial class ProtocolHelpWindow : Window
{
    public ProtocolHelpWindow()
    {
        ...
        Body("五协议可随时在主界面顶部“协议”下拉切换；以下 X 指 X 轴取值，Y 指曲线数值。");
...
""");
        Body("注意：仅解析应答帧（请求帧可用发送区/命令面板发送）；同一帧内数据类型必须一致。CRC 也可用“工具箱→数据校验”在线核对。");
    ...
    private void Body(string text)
        ...
        {
            Text = text,
            Foreground = new SolidColorBrush(Color.FromRgb(0xD6, 0xE2, 0xEE)),
            FontSize = 12.5,
            TextWrapping = TextWrapping.Wrap,
            Margin = new Thickness(0, 2, 0, 2),
        });

    private void Sample(string text) => AddBlock(text, new SolidColorBrush(Color.FromRgb(0x18, 0x22, 0x2E)));

    private void Code(string text) => AddBlock(text, new SolidColorBrush(Color.FromRgb(0x10, 0x13, 0x17)));

    private void AddBlock(string text, Brush background)
        => ContentPanel.Children.Add(new TextBox
        {
            ...
            Foreground = new SolidColorBrush(Color.FromRgb(0xC9, 0xE3, 0xB0)),
            BorderBrush = new SolidColorBrush(Color.FromRgb(0x2A, 0x3A, 0x4D)),
